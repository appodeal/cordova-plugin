...
module.exports = function (context) {
    ...
    function parsePluginXmls() {
        ...
        context.opts.cordova.plugins.forEach(function (id) {
            ...
            parser.parseString(fs.readFileSync('plugins/com.appodeal.plugin/plugin.xml'), function (err, data) {
                        ...
                        data.plugin.platform.forEach(function (platform) {
                            ...
                            if (platform.$.name === 'ios') {
                                if (platform.$.name === 'ios') {
                                    ...
                                    (platform['pod-source'] || []).forEach(function (source) {
                                        sourceID++;
                                        newSources.sources[sourceID] = source.$;
                                        console.log('%s requires pod source: %s', id, source.$['url']);
                                    });
    ...
    function createFiles() {
        ...
        if (!podified || !_.isEqual(newPods, currentPods)) {
            ...
            for (sourceId in newSources.sources) {
                var source = newSources.sources[sourceId];
                if(source['url']) {
                    podfileContents.push("source '" + source['url'] + "'");
                }
            }
